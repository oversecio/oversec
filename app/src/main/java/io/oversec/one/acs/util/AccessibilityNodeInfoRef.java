 ...
 * <p>
 * This class keeps track of an {@link AccessibilityNodeInfo}
 ...
 *
 * @see AccessibilityNodeInfoUtils#isVisibleOrLegacy(AccessibilityNodeInfo)
 */
public class AccessibilityNodeInfoRef {
     ...
     */
    public void clear() {
        reset((AccessibilityNodeInfo) null);
    }
     ...
     * new node.
     */
    public void reset(AccessibilityNodeInfo newNode) {
    ...
    public static AccessibilityNodeInfoRef obtain(
        ...
        return new AccessibilityNodeInfoRef(
                AccessibilityNodeInfo.obtain(node), true);
    }
    ...
    public AccessibilityNodeInfoRef makeOwned() {
        if (mNode != null && !mOwned) {
            reset(AccessibilityNodeInfo.obtain(mNode));
        }
        return this;
    }
    ...
    private AccessibilityNodeInfoRef(AccessibilityNodeInfo node,
                                     boolean owned) {
        mNode = node;
        mOwned = owned;
    }
     ...
     */
    public AccessibilityNodeInfo release() {
        mOwned = false;
        return mNode;
    }
    ...
    public boolean parent() {
        ...
        AccessibilityNodeInfo parentNode = mNode.getParent();
        while (parentNode != null) {
            if (AccessibilityNodeInfoUtils.isVisibleOrLegacy(parentNode)) {
                reset(parentNode);
                return true;
            }
            AccessibilityNodeInfo tmp = parentNode.getParent();
            parentNode.recycle();
            parentNode = tmp;
        }
        return false;
    ...
    private int getChildNumber(AccessibilityNodeInfo parent) {
        int ret = -1;
        int childCount = parent.getChildCount();
